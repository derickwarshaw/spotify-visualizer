...
function calcUsefulYCoords(){var yMin=(Math.max(allYPoints)-Math.min(allYPoints))*0.8;if(mountain.geometry.vertices[k].y>yMin){usefulYCoords.push(k)}}
function animateMountainHeight(){if(!mountain){return;}
for(var j=0;j<usefulYCoords.length;j++){mountain.geometry.vertices[usefulYCoords[j]].y+=getRandomInt(-3,3)
