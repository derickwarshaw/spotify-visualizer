...
function calcZCoords(){
    ...
    for(var k = 0; k < mountain.geometry.vertices.length; k++){
        console.log(mountain.geometry.vertices[k].y)
        if(mountain.geometry.vertices[k].y > 5) {
            console.log(mountain.geometry.vertices[k].y)
            zPoints.push(k)
        }
    }
...
function animateMountainHeight(){
    ...
    for(var j = 0; j < zPoints.length; j++){
        mountain.geometry.vertices[zPoints[j]].y += getRandomInt(-3, 3)
